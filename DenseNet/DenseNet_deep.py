...
#         current = conv2dReduce(current,3,16,2)
        ...
        final_dim = features + csv_dim
    
    
        print current.get_shape
        print ys.get_shape
        current = tf.concat([current,tf.reshape(zs,[-1,1,1,csv_dim])],3)
...
def run():
    ...
    train_imageData,train_csvData,train_labels = my_load('./npy/aoeiu_train_shape_muscleLen_physiology.npy', '.')
    test_imageData,test_csvData,test_labels = my_load('./npy/aoeiu_test_shape_muscleLen_physiology.npy', '.')
